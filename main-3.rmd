...
In this project we're working with a dataset which contains the house prices for King County, Seattle, USA. It includes houses sold between May 2014 and May 2015. The dataset consist of 21.613 observations of 21 variables, which is:
...
coordinate <- as.matrix(data.frame(lon = kc$long, 
                                   lat = kc$lat))
...
# Vær helt sikker på at det er dette der gøres og menes: Next we split the price of the housesale into 15 intervals where the mean price is calculated for each. Afterwards the density of the each interval is plotted in the the map of King County.
...
DISKUSSION - skal dette med eller ej 
...
kc %>% filter(bedrooms!=33) %>% select(price, grade, sqft_living, sqft_lot, view, waterfront, floors, bedrooms, bathrooms, condition, sqft_above, sqft_basement, sqft_living15, sqft_lot15) %>%  
...
cat(paste0("Size of training set: ", nrow(training)," of 21613\n","\n",
           ...
           "Total:          ", round(difftime(t10,t0, units = "mins"),2),"\n"))
```
...
df_mae <- cbind(mae_glm, mae_lar, mae_ela, mae_tre, mae_raf, mae_nnt, mae_ran, mae_xgb, mae_knn, mae_bag, mae_cub)
...
df_mae %>% 
  ...
  mutate(Model = c("LinearModel", "LARS","ElasticNet", "RegressionTree", "RandomForrest", 
                   "NeuralNet","RandomAssignment", "XGB", "KNN","BaggedTree","Cubist")) %>% 
  ggplot(aes(reorder(Model, desc(mae)), mae/1000)) + 
  geom_col() +
  geom_text(aes(y=(mae/1000)-10, label=format(round(mae/1000,1)), nsmall = 1), color="white") +
...
df_rmse <- cbind(rmse_glm, rmse_lar, rmse_ela, rmse_tre, rmse_raf, rmse_nnt, rmse_ran, rmse_xgb, rmse_knn, rmse_bag, rmse_cub)
...
```{r}
Price <- createDataPartition(kc$price, p = 0.8, list = FALSE, times = 1)

kc_train  <- kc[ Price,] %>%  select(-c(date,id))
kc_test   <- kc[-Price,] %>%  select(-c(date,id))

x_train <- kc_train
x_test <- kc_test

model_recipe <- recipe(price ~ ., data = x_train)

model_recipe_steps <- model_recipe %>% 
  step_num2factor(bedrooms,condition, view, grade, waterfront, floors, yr_built, yr_renovated, basement, zipcode, cluster) %>% 
  step_dummy(bedrooms,condition, view, grade, waterfront, floors, yr_built, yr_renovated, basement, zipcode, cluster) %>%
  step_range(bathrooms, sqft_living, sqft_lot, sqft_above, sqft_basement, sqft_living15, sqft_lot15, distance, min = 0, max = 1)

prepped_recipe <- prep(model_recipe_steps, training = x_train)

x_train_recipe <- bake(prepped_recipe, x_train)
y_train_recipe <- x_train_recipe %>% select(price) %>% as.data.frame()
x_train_recipe <- x_train_recipe %>% select(-price)

x_test_recipe <- bake(prepped_recipe, x_test)
y_test_recipe <- x_test_recipe %>% select(price) %>% as.data.frame()
x_test_recipe <- x_test_recipe %>% select(-price)

model <- keras_model_sequential() %>%
  layer_dense(units = 431, activation = "relu", input_shape = dim(x_train_recipe)[2]) %>%
...
hist<-model %>% keras::fit(as.matrix(x_train_recipe), as.matrix(y_train_recipe),
     epochs = epochs,
  validation_split = 0.2,
  batch_sieze = 32,
  verbose = 1)

t <- model %>% evaluate(as.matrix(x_test_recipe),as.matrix(y_test_recipe))
