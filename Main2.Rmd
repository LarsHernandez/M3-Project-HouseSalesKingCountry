...
pred_knn <- predict(fit_knn,  newdata=test) %>% as.vector
mae_knn <- MAE(pred_knn, test$price)

pred_bag <- predict(fit_bag,  newdata=test) %>% as.vector
mae_bag <- MAE(pred_bag, test$price)

pred_cub <- predict(fit_cub,  newdata=test) %>% as.vector
mae_cub <- MAE(pred_cub, test$price)


kable(cbind(mae_glm, mae_lar, mae_ela, mae_tre, mae_raf, mae_nnt, mae_ran, mae_xgb, mae_knn, mae_bag, mae_cub)) %>% 
  kable_styling("bordered", "condensed")

rbind(mae_glm, mae_lar, mae_ela, mae_tre, mae_raf, mae_nnt, mae_ran, mae_xgb, mae_knn, mae_bag, mae_cub) %>% 
  as_tibble %>% 
  rename(mae = V1) %>% 
